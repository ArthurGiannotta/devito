...
from devito.implicit import IterativeMethod
...
class Operator(Callable):
    ...
    **kwargs
    ...
    def __init__(self, expressions, **kwargs):
        ...
        # Process expressions with more than one term in the left hand side
        iterative_method = kwargs.get("iterative_method", IterativeMethod.SOR)
        expressions = self._process_implicit_lhs(expressions, iterative_method)
    ...
    def _process_implicit_lhs(self, expressions, method):
        processed = []

        for e in expressions:
            if e.lhs.is_Add:
                processed += method.explicit_expressions(e)
            else:
                processed += [e]
